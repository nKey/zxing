...
package zxing.datamatrix.detector;

import zxing.NotFoundException;
import zxing.ResultPoint;
import zxing.common.BitMatrix;
import zxing.common.DetectorResult;
import zxing.common.GridSampler;
import zxing.common.detector.MathUtils;
import zxing.common.detector.WhiteRectangleDetector;
...
public final class Detector {
	...
	public DetectorResult detect() throws NotFoundException {
		...
		int dimensionTop = transitionsBetween(topLeft, topRight)
				.getTransitions();
		...
		if (4 * dimensionTop >= 7 * dimensionRight
			...
			correctedTopRight = correctTopRightRectangular(bottomLeft,
					bottomRight, topLeft, topRight, dimensionTop,
					dimensionRight);
			if (correctedTopRight == null) {
				correctedTopRight = topRight;
			}

			dimensionTop = transitionsBetween(topLeft, correctedTopRight)
					.getTransitions();
			dimensionRight = transitionsBetween(bottomRight, correctedTopRight)
					.getTransitions();
			...
			bits = sampleGrid(image, topLeft, bottomLeft, bottomRight,
			...
			correctedTopRight = correctTopRight(bottomLeft, bottomRight,
			...
			int dimensionCorrected = Math.max(
					transitionsBetween(topLeft, correctedTopRight)
							.getTransitions(),
	...
	private boolean isValid(ResultPoint p) {
		return p.getX() >= 0 && p.getX() < image.getWidth() && p.getY() > 0
	 ...
	 */
	private static void increment(Map<ResultPoint, Integer> table,
			ResultPoint key) {
	...
	private static BitMatrix sampleGrid(BitMatrix image, ResultPoint topLeft,
			...
			ResultPoint topRight, int dimensionX, int dimensionY)
			throws NotFoundException {

		GridSampler sampler = GridSampler.getInstance();

		return sampler.sampleGrid(image, dimensionX, dimensionY, 0.5f, 0.5f,
				dimensionX - 0.5f, 0.5f, dimensionX - 0.5f, dimensionY - 0.5f,
				0.5f, dimensionY - 0.5f, topLeft.getX(), topLeft.getY(),
	...
	private static final class ResultPointsAndTransitionsComparator implements
		...
		@Override
		public int compare(ResultPointsAndTransitions o1,
				ResultPointsAndTransitions o2) {
